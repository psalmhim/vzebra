...
The model comprises 7,282 Izhikevich spiking neurons with cell-type-specific
membrane dynamics (RS, IB, FS, CH, LTS, TC, MSN) distributed across 41 brain
...
A validated test suite of 305 assertions covers all 48 brain modules,
...
Each of the 7,190 model neurons is assigned a 3D position sampled from a
...
\end{equation}
aggregating 292 two-compartment neurons across 23 modules:
...
Nociception & 3 (mech, therm, chem) & pain signals        & 12 \\
Auditory    & 3 (low, mid, high)  & frequency bands       & 12 \\
Gustatory   & 3 (amino, bitter, umami) & taste channels   & 12 \\
Tactile     & 3 (head, trunk, tail) & somatotopic contact & 12 \\
Hypothalamus& 5 (LH, VMH, PVN, POA, SCN) & homeostatic state & 20 \\
Pineal      & 2 (photo, melatonin)  & light \& melatonin  & 8  \\
Inferior olive& 2 (error, timing)   & climbing fiber PE   & 8  \\
Dl pallium    & 4 (DG, CA3, CA1, SWR) & episodic memory   & 16 \\
Vagus nerve & 2 (visceral, cardiac) & visceral afferent PE & 8 \\
Pituitary   & 3 (ant, inter, post)  & endocrine PE       & 12 \\
Area postrema & 2 (chemo, metabolic) & blood toxin PE     & 8 \\
NTS         & 2 (gustatory, visceral) & visceral relay PE  & 8 \\
Lat.\ line efferent & 2 (flow pred, suppr) & corollary discharge PE & 8 \\
...
\subsection{Additional Sensory Modalities}

Four sensory systems complete the zebrafish sensorium.
Each is a 6-neuron RS population with a 3-channel TwoCompColumn.

\paragraph{Nociception (TRPA1/TRPV1, Rohon-Beard neurons).}
Three channels encode mechanical (collision, predator bite at $<$40\,px),
thermal (TRPV1 above 33\textdegree{}C, cold below 15\textdegree{}C), and
chemical (TRPA1 irritants) pain.
Sensitisation ($\times$2 max after repeated strong pain) and habituation
($\times$0.5 min after repeated mild stimuli) operate on a 20-step
sliding window.
Pain $>$0.3 triggers a withdrawal reflex reducing swimming speed.

\paragraph{Auditory system (Weberian apparatus).}
Three frequency bands encode predator swim sounds (low $<$200\,Hz,
intensity $\propto (1-d/200)^2$), conspecific vocalizations (mid
200--1000\,Hz), and ambient/sudden sounds (high $>$1000\,Hz).
A sudden loud onset ($>$0.6) or abrupt low-frequency increase triggers
Mauthner-cell acoustic startle with 10-step refractory suppression.

\paragraph{Gustatory system (T1R/T2R, vagal lobe).}
Three taste channels encode amino acids (appetitive, T1R), bitter
compounds (aversive, T2R), and umami (nutrient).
Palatability $= (\text{amino}+\text{umami})/2 - \text{bitter}$; negative
values during feeding trigger a spit reflex.

\paragraph{Tactile/somatosensory system (trigeminal, Rohon-Beard).}
Three somatotopic channels encode head (trigeminal ganglion), trunk
(Rohon-Beard neurons), and tail mechanoreception.
Efference copy suppresses self-generated tail-beat signals by 80\%.
Head touch $>$0.5 triggers a tactile startle reflex.

\subsection{Brain Organ Modules}

Three subcortical organ modules complete the cerebellar
error-signalling pathways, adding 24 spiking neurons across 3 regions.

\paragraph{Hypothalamus (LH, VMH, PVN, POA, SCN).}
The hypothalamus serves as the master homeostatic integrator, with five
nuclei implemented as distinct TwoCompColumn channels:
(i)~lateral hypothalamus (LH): hunger/feeding drive scaled by energy deficit;
(ii)~ventromedial hypothalamus (VMH): satiety signal opposing LH;
(iii)~paraventricular nucleus (PVN): stress-responsive CRH release driving
the HPA axis and modulating noradrenaline;
(iv)~preoptic area (POA): thermoregulatory set-point comparator;
(v)~suprachiasmatic nucleus (SCN): circadian pacemaker that entrains
melatonin synthesis in the pineal gland.
The five-channel column (20 neurons, RS type) receives interoceptive
afferents (energy, temperature, cortisol) on the soma and top-down
goal-state predictions on the apical dendrite.

\paragraph{Pineal gland (direct photoreception + melatonin).}
Two channels encode (i)~direct photoreception via pineal-specific
opsins (parapinopsin, exo-rhodopsin) and (ii)~melatonin synthesis
rate, which is high during darkness and suppressed by light.
The 8-neuron RS population (2-channel TwoCompColumn) receives SCN
timing from the hypothalamus on the apical compartment and ambient
light intensity on the soma.  Melatonin output modulates serotonin
baseline (inverse relationship) and suppresses locomotor drive at night.

\paragraph{Inferior olive (climbing fiber error signal).}
Two channels encode (i)~sensorimotor prediction error (mismatch between
expected and actual proprioceptive feedback) and (ii)~temporal error
(timing discrepancy in rhythmic motor sequences).  The 8-neuron RS
population (2-channel TwoCompColumn) projects climbing fibers to the
cerebellum, where complex spikes drive supervised learning of the
cerebellar internal model.  Inferior olive firing is suppressed by
cerebellar nucleo-olivary inhibition, implementing the classic
Marr--Albus--Ito error-correction loop.

\paragraph{Dorsolateral pallium (Dl, hippocampus homolog).}
...
\subsection{Autonomic and Visceral Modules}

Five additional spiking modules extend the model's autonomic and
visceral processing pathways, adding 48 spiking neurons across 5 regions.

\paragraph{Vagus nerve (bidirectional visceral communication).}
The spiking vagus nerve (10 RS neurons: 4 afferent, 4 efferent,
2 cardiac) implements the primary parasympathetic pathway between the
brainstem and visceral organs.
Afferent fibres relay gut distension, cardiovascular baroreceptor,
and respiratory signals to the NTS; efferent fibres modulate heart rate
and digestive motility via parasympathetic tone.
A 2-channel TwoCompColumn (8 neurons) computes visceral afferent and
cardiac prediction errors, enabling interoceptive active inference
whereby the brain predicts---and acts to fulfil---expected visceral states.

\paragraph{Pituitary (master endocrine organ).}
The spiking pituitary (12 RS neurons: 4 anterior, 4 intermediate,
4 posterior) models the three-lobed master endocrine organ.
Anterior cells release GH and prolactin analogues under hypothalamic
releasing-factor control; intermediate cells produce $\alpha$-MSH for
pigmentation and stress responses; posterior cells store and release
vasotocin (AVP homologue) and isotocin (OT homologue).
A 3-channel TwoCompColumn (12 neurons) generates prediction errors
for each lobe, enabling neuroendocrine set-point adaptation through
variational free energy minimization.

\paragraph{Area postrema (circumventricular chemosensor).}
...
behavior that reduces ingestion of harmful substances.
A 2-channel TwoCompColumn (8 neurons) computes chemosensory and
metabolic prediction errors.

\paragraph{Nucleus of the solitary tract (NTS, primary visceral relay).}
The spiking NTS (10 RS neurons: 4 gustatory, 3 visceral,
3 cardiorespiratory) serves as the primary visceral sensory relay
in the brainstem, integrating convergent input from the vagus,
glossopharyngeal, and facial nerves.
Gustatory neurons receive taste input relayed from palatal and
pharyngeal receptors; visceral neurons integrate vagal afferents
from gut and liver; cardiorespiratory neurons gate baroreflex
sensitivity and respiratory rhythm generation.
A 2-channel TwoCompColumn (8 neurons) computes gustatory and visceral
prediction errors that project to the hypothalamus for homeostatic control.

\paragraph{Lateral line efferent (corollary discharge suppression).}
...
efference copy.
During active swimming, motor commands generate corollary discharge
signals that suppress lateral line hair cell sensitivity, preventing
self-generated flow signals from triggering false predator alarms.
A 2-channel TwoCompColumn (8 neurons) computes flow prediction error
and suppression error, enabling adaptive gain control of the afferent
lateral line system.

\subsection{Action-Perception Cycle}

The model implements Friston's (2011) full action-perception cycle:
motor commands are proprioceptive \emph{predictions} that the spinal
reflex arc acts to fulfil.  The cycle runs within each time step via
iterative inference:

\begin{enumerate}[nosep]
  \item \textbf{Predict}: Generate goal-dependent proprioceptive predictions
        $\boldsymbol{\mu}^{(0)}$ (top-down $\to$ apical dendrite of
        48 active-motor two-compartment neurons).
  \item \textbf{Sense}: Encode actual proprioception
        $\mathbf{y}$ (bottom-up $\to$ soma).
  \item \textbf{Infer}: For $k = 1 \ldots K$ passes ($K=3$):
        \begin{equation}\label{eq:apc_update}
          \boldsymbol{\varepsilon}^{(k)} = V_a^{(k)} - \tilde{V}_s, \qquad
          \boldsymbol{\mu}^{(k)} \leftarrow \boldsymbol{\mu}^{(k-1)}
            - \eta_\text{act}\,\boldsymbol{\varepsilon}^{(k)},
        \end{equation}
        where $\eta_\text{act}=0.15$.  Each pass runs the TwoCompColumn
        forward, updating apical beliefs and precision simultaneously.
  \item \textbf{Act}: Motor output = reflex-arc readout of final PE:
        $\delta\theta = 0.6\,\varepsilon_\text{heading}
                       + 0.4\,\varepsilon_\text{turn\_rate}$;
        $v = \mu_\text{speed} + 1.5\,\varepsilon_\text{speed}$.
\end{enumerate}

\paragraph{Adaptive blend.}
The active-inference motor is blended with the reactive motor via
a precision-dependent blend factor:
\begin{equation}\label{eq:ai_blend}
  \alpha_\text{AI} = \text{clamp}\bigl(
    \alpha_0 + 0.4\,\bar{\pi}_\text{motor} - 0.3\,\xi,\;
    0.1,\; 0.8 \bigr),
\end{equation}
where $\alpha_0=0.3$ is the base blend, $\bar{\pi}_\text{motor}$ is
the mean precision of the active-motor column, and $\xi \in [0,1]$
is the \emph{spontaneity} factor (see below).  High precision $\Rightarrow$
the generative model is trusted $\Rightarrow$ more active inference.

\paragraph{Free-energy gradient goal modulation.}
The temporal derivative of brain-level free energy,
$\dot{F} = F_t - F_{t-1}$, modulates goal selection:
if $\dot{F} > 0.05$ (surprise rising under the current goal), the
current goal's EFE is penalised by $0.3\,\dot{F}$, encouraging
the WTA goal selector to switch.  This closes the loop: free energy
drives not only perception (belief updating) but also action (goal
switching).

\paragraph{Spontaneity: anti-FEP mechanism.}
Real organisms do not always minimise free energy.
Spontaneous exploration, play, frustration-driven switching, and
intrinsic neural noise all produce behaviour that temporarily
\emph{increases} prediction error.  We model this as:
\begin{equation}\label{eq:spontaneity}
  \xi = \text{clamp}\!\bigl(
    0.4\,h_\text{hab} + 0.25\,\mathbb{1}_{|\dot{F}|<0.02,\,\nu<0.3}
    + 0.8\max(0, \text{DA}-0.6) + 0.15\,\mathbb{1}_\text{social}
    + 0.05,\; 0,\; 1 \bigr),
\end{equation}
where $h_\text{hab}$ is habenula helplessness (Hikosaka 2010),
$\nu$ is novelty, and $\text{DA}$ is the dopamine level.
High spontaneity (i) reduces the active-inference blend via
Eq.~\ref{eq:ai_blend}, falling back to reactive control,
and (ii) injects stochastic motor noise when $\xi > 0.3$:
$\delta\theta \mathrel{+}= \mathcal{U}(-0.5, 0.5)\,\cdot\,0.7(\xi - 0.3)$.

This creates a natural tension between the mechanistic FEP drive
(precision-weighted PE minimisation) and the organism's need to break
free of local minima---mediated by neuromodulatory state rather than
an external exploration schedule.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/v2_paper/fig_apc.pdf}
\caption{Action-perception cycle diagnostics over 400 steps.
(a) Iterative inference reduces free energy within each step:
median $F$ across inference passes $1\!\to\!3$ (shaded IQR, gray
traces individual time points).  Mean reduction $\Delta F \approx 0.035$
per step.
(b) Adaptive blend $\alpha_\text{AI}$ (orange, left axis) tracks
mean motor precision $\bar{\pi}$ (navy, right axis); $\alpha$ stays
within $[0.43, 0.56]$ in steady state, well above the fixed baseline
$\alpha = 0.3$ (gray dashed) when precision is healthy.
(c) Spontaneity $\xi$ (crimson): baseline $\xi = 0.05$ from neural
noise; rises above the $0.3$ threshold (red shaded) when habenula
helplessness or phasic-DA exploration is engaged.
(d) $\dot{F} = dF/dt$ (purple): when surprise rises above
$0.05$ (dashed), the current goal is penalised in the WTA selector;
coloured bands at the bottom indicate goal identity
(FORAGE, FLEE, EXPLORE, SOCIAL).}
\label{fig:apc}
\end{figure}

\paragraph{APC ablation.}
Comparing $K\!=\!3$ inference passes (APC ON) against a single-pass
baseline ($K\!=\!1$, fixed $\alpha\!=\!0.3$, no spontaneity) over
$n\!=\!5$ matched seeds (300 steps each):
...
\noindent
Iterative inference reduces mean $F$ by 78\% without regressing
food intake or survival.  The adaptive blend $\alpha_\text{AI}$
responds to precision state, rising above the fixed baseline when
the generative model is trusted, and falling when spontaneity
(anti-FEP) overrides mechanistic control.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/v2_paper/fig5_interoception.pdf}
\caption{\textbf{Interoceptive signals.}
(a)~Emotional valence (positive during foraging, negative during threat).
(b)~Arousal (elevated during predator encounters and starvation).
(c)~Heart rate (cardiorespiratory coupling via insula module).
(d)~Energy/hunger level (allostatic regulation drives foraging urgency).
The interoceptive insula integrates physiological state with emotional
valence to modulate goal selection via the EFE anticipatory-anxiety term.}
\label{fig:interoception}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../plots/v2_paper/fig6_goal_distribution.pdf}
\caption{\textbf{Goal distribution across training.}
Fraction of time spent in each goal state (forage, flee, explore, social)
as a function of training round.  Early training: high exploration.
Mid training: increasing forage efficiency.  Late training: balanced
policy with reduced unnecessary flee responses.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../plots/v2_paper/fig10_place_cells.pdf}
\caption{\textbf{Place cells and spatial memory.}
(a)~Geographic model heat map showing learned food-probability landscape
across the 10$\times$8 spatial grid after 85 training rounds.
(b)~Theta-phase precession: spike phase advances as the fish traverses
a place field, enabling sequence-level spatial coding.
(c)~Food-associated place cells develop elevated firing rates
relative to arena-average.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/v2_paper/fig1_architecture.pdf}
\caption{\textbf{Architecture overview.}
Block diagram of the 48-module zebrafish brain showing the complete
sensory--motor loop.  Blue: excitatory spiking pathways.  Red: inhibitory.
Orange: neuromodulatory.  Green: plasticity (STDP, Hebbian).
Fourteen sensory and organ modalities (retina, lateral line, olfaction, vestibular,
proprioception, color vision, binocular depth, nociception, auditory,
gustatory, tactile, hypothalamus, pineal gland, inferior olive) feed through
the thalamo-pallial loop to basal ganglia action selection and reticulospinal
motor output.}
...
Dl pallium      & RS          &  15     &  5  & Place cells + salience       & 9 \\
Place cells     & RS+IB       & 128     & 32  & Path integration + visual    & 9 \\
\midrule
\textbf{Total} & & \textbf{3,247} & \textbf{780} & & \\
\midrule
\textbf{Grand total} & & \multicolumn{2}{c}{\textbf{$\sim$4,027}} & & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/v2_paper/fig2_training_dynamics.pdf}
\caption{\textbf{Training dynamics.}
Checkpoint-resolved training curves across 85 rounds of online learning.
(a)~Mean survival (steps per episode).
(b)~Cumulative food intake.
(c)~Classifier accuracy on food/enemy/rock categories.
(d)~Mean free energy (variational surprise).
Shaded bands show $\pm$1\,SD across seeds.}
\label{fig:training_dynamics}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/v2_paper/fig9_v1v2.pdf}
\caption{\textbf{v1 vs.\ v2 performance comparison.}
Matched-seed comparison of survival duration, food intake, classifier
accuracy, and decision quality.  v2 improves mean survival by 32\%
(319$\to$422 steps) and food intake by 156\% (3.0$\to$7.7 items).}
\label{fig:v1v2_comparison}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../plots/v2_paper/fig7_survival.pdf}
\caption{\textbf{Multi-seed survival analysis.}
Survival duration across 9 independent seeds ($474 \pm 72$ steps).
8/9 seeds reach the full 1500-step episode.  Bars show individual seed
performance; error bar denotes mean $\pm$1\,SD.}
\label{fig:survival_multiseed}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/v2_paper/fig3_raster.pdf}
\caption{\textbf{Spike raster across brain layers.}
Representative 500-step episode showing spiking activity across all
major populations: retina (ON/OFF/loom/DS), optic tectum (SFGS-b/d,
SGC, SO), thalamus (TC, TRN), pallium (S, D), basal ganglia (D1, D2,
GPi), reticulospinal, and spinal CPG.  Vertical gray bands mark flee
episodes; green ticks mark food consumption events.  E/I balance
produces sparse coding (10--20\% active) with emergent gamma
oscillations in the thalamo-pallial loop.}
\label{fig:spike_raster}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/v2_paper/fig4_neuromod.pdf}
\caption{\textbf{Neuromodulatory dynamics.}
Four-axis neuromodulation (DA, NA, 5-HT, ACh) and Expected Free Energy
(EFE) traces over a representative episode.
(a)~DA rises after food reward and during successful foraging.
(b)~NA spikes during predator encounters (threat-driven arousal).
(c)~5-HT declines during sustained flee (frustration $\to$ patience loss).
(d)~ACh tracks circadian$\times$fatigue gating.
(e)~EFE across four goal policies showing real-time policy competition.}
\label{fig:neuromod_traces}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../plots/v2_paper/fig11_classifier.pdf}
\caption{\textbf{Object classifier performance.}
(a)~Classifier accuracy across training rounds (converges to 96.2\%).
(b)~Confusion matrix on held-out gameplay frames:
food (green), enemy (red), rock (gray), empty (white).
Online fine-tuning with ACh-gated food learning and NA-gated enemy
learning progressively sharpens category boundaries.}
\label{fig:classifier}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/v2_paper/fig8_ablation.pdf}
\caption{\textbf{Ablation study.}
Survival and food intake when each of 10 brain regions is individually
ablated (set to zero output during execution).
Tectum ablation is most devastating (eliminates vision $\to$
near-zero survival).  Pallium and basal ganglia ablation severely
impairs goal-directed behavior.  Cerebellum and habenula ablation
produces modest effects, consistent with their modulatory rather
than essential roles.}
\label{fig:ablation}
\end{figure}
...
Table~\ref{tab:test_suite} summarizes all test levels; 392 assertions
pass across all 48 brain modules.
...
\texttt{step11\_untested\_modules.py}  & 12 modules (60 vision, 12 motor) & 72 & PASS \\
\texttt{step12\_sensory\_depth.py}     & Color vision, vestibular, proprioception & 24 & PASS \\
\texttt{step13\_binocular\_depth.py}   & Binocular stereo (9 categories) & 31 & PASS \\
\texttt{step14\_missing\_senses.py}    & Nociception, auditory, gustatory, tactile & 32 & PASS \\
\texttt{step15\_organ\_modules.py}    & Hypothalamus, pineal, inferior olive & 26 & PASS \\
\texttt{step16\_dl\_pallium.py}      & Dl pallium episodic memory (DG/CA3/CA1/SWR) & 30 & PASS \\
\texttt{step17\_autonomic\_modules.py} & Vagus, pituitary, area postrema, NTS, LL efferent & 35 & PASS \\
\texttt{zebrav1/step35\_42\_late\_modules.py} & V1 steps 35--42 & 31 & PASS \\
\midrule
\textbf{Total} & & \textbf{432} & \textbf{ALL PASS} \\
\bottomrule
\end{tabular}
\end{table}
...
%===================================================================
\section{Multi-Species Validation Platform (vzlab)}\label{sec:vzlab}

\subsection{Framework Overview}

The v2 zebrafish brain is the reference implementation for
\textsc{vzlab}, a cross-species virtual laboratory platform that
validates neural avatars against biological ground truth at four
hierarchical tiers.
The platform provides species-agnostic abstract base classes
(\texttt{Connectome}, \texttt{BrainModule}, \texttt{Environment},
\texttt{BehavioralProtocol}) wired by a single
\texttt{ExperimentRunner} API.

\subsection{Four-Tier Validation Protocol}

\begin{table}[h]
\centering
\caption{vzlab validation tiers.}
\begin{tabular}{lll}
\toprule
Tier & Method & Pass criterion \\
\midrule
T1 & Behavioural battery & Species-specific thresholds \\
T2 & Spearman rank atlas correspondence & $r_s > 0.50$ \\
T3 & Lesion replication & Ablation reproduces published deficit \\
T4 & Sensory dropout robustness (10/30/50\%) & 80/50/20\% activity retention \\
\bottomrule
\end{tabular}
\end{table}

\noindent Tier~4 applies Bernoulli dropout directly to the sensory
signal struct before \texttt{brain.step()}, so the avatar makes
decisions from genuinely degraded input---not masked observations.
The robustness index $\text{RI} = \text{AUC}(\text{retention vs.\ dropout fraction})$,
normalised to $[0,\,1]$ over the range $[0,\,0.5]$.
Grade~A+ requires all four tiers to pass simultaneously.

\subsection{Species Results}

\begin{table}[h]
\centering
\caption{Multi-species vzlab validation results.}
\begin{tabular}{lrllll}
\toprule
Species & Neurons & T1 & T2 $r_s$ & T3 & T4 RI \\
\midrule
\textit{Danio rerio} (zebrafish v2) & 7,316 & PASS & 0.81 & PASS & 1.000 \\
\textit{C.\ elegans}                 & 302   & PASS & 0.72 & PASS & 0.960 \\
\textit{Drosophila melanogaster}     & 2,145 & PASS & 0.68 & PASS & 0.946 \\
\textit{Xenopus laevis} tadpole      & 150   & PASS & 1.00 & PASS & 1.000 \\
\bottomrule
\end{tabular}
\end{table}

\noindent All four species achieve Grade~A+, spanning three phyla and
four orders of magnitude in neuron count (150--7,316).

\subsection{STDP Synaptic Dropout}

To build fault-tolerant distributed representations---matching
biological synaptic unreliability ($\sim$10--20\% release
failure per terminal)---each \texttt{EligibilitySTDP.consolidate()}
call applies a Bernoulli dropout mask to the weight delta $\Delta W$
before committing:

\begin{equation}
\Delta W_\text{eff} = \Delta W \odot \mathbf{m}, \quad
m_{ij} \sim \text{Bernoulli}(1 - p_\text{drop}), \quad
p_\text{drop} = 0.10.
\end{equation}

This prevents synapse co-adaptation and forces weight redundancy.
The parameter is exposed as \texttt{PlasticityConfig.stdp\_dropout\_p}
(default 0.10) so ablation studies can sweep over it.

\subsection{Social Behaviour Validation}

The zebrafish Tier~1 battery was extended with two multi-agent tests
that complete the cells$\to$circuits$\to$behaviour$\to$social chain:

\begin{itemize}
\item \textbf{Shoaling cohesion} (3~agents, 60~steps): mean
  pairwise cohesion $\geq 0.20$ (measured as
  $1 - \bar{d}/d_\text{arena}$).
  Driven by \texttt{SocialField} proximity signals.
\item \textbf{Social alarm propagation} (3~agents, alarm stimulus
  at step~20): mean propagation fraction $\geq 0.50$, testing
  chemical alarm relay through conspecific detection.
\end{itemize}

Both tests pass, confirming emergent group-level behaviour from
individually implemented neural circuits.
...
A complete 432-assertion test suite validates all 48 modules
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../plots/v2_paper/fig_fe_trace_wt_vs_scz.pdf}
\caption{\textbf{Free energy traces: wildtype vs.\ schizophrenia phenotype.}
Brain-level variational free energy $F_\text{brain}$ over a 500-step
episode.  Wildtype (blue) maintains low, stable free energy with
transient spikes during predator encounters.  Schizophrenia phenotype
(red, $\pi_\text{max} \leq 0.18$) shows elevated baseline free energy
and impaired prediction-error suppression, consistent with the
aberrant salience hypothesis~(Kapur, 2003).}
\label{fig:fe_disorder}
\end{figure}
...
\begin{itemize}[nosep]
  \item \textbf{BrainConfig} --- 7 sub-dataclasses ($>$200 parameters):
    ...
    ablation (48 boolean region toggles),
...
Key features:
\begin{itemize}[nosep]
  \item \textbf{Lesion/Enable}: toggles any of 48 brain regions via \texttt{AblationConfig}
