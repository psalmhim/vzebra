...
\title{%
  \textbf{Virtual Zebrafish v2: A Fully Spiking, Excitatory--Inhibitory\\
  Brain Model with Four-Axis Neuromodulation and\\
  Connectome-Constrained Wiring}\\[8pt]
  \large Architecture Design and Roadmap --- Zebrafish SNN v2
}
...
%===================================================================
\begin{abstract}
We present the design specification for \textbf{ZebrafishSNN v2}, a substantially
more biologically plausible successor to v1 that addresses four fundamental
limitations of the existing architecture.
First, v1 uses continuous rate-coded leaky integrators; v2 replaces all neurons
with Izhikevich conductance-based spiking units with cell-type-specific
membrane dynamics (RS, IB, FS, CH) and true millisecond-resolution spike timing.
Second, v1 contains no inhibitory interneurons; v2 adds 25\% fast-spiking (FS)
GABAergic interneurons to every layer, restoring excitatory--inhibitory balance,
enabling sparse coding, oscillatory dynamics, and eliminating signal death.
Third, v1 uses a single dopamine axis for neuromodulation; v2 implements
four independent neuromodulatory systems---dopamine (DA), noradrenaline (NA),
serotonin (5-HT), and acetylcholine (ACh)---each with distinct source nuclei,
projection targets, and functional roles consistent with zebrafish
neuroanatomy~\cite{bhatt2009,orger2008}.
Fourth, v1 relies on rate-based Hebbian STDP; v2 adopts three-factor
reward-modulated STDP with millisecond-resolution eligibility traces
($\tau_e = 500$\,ms), enabling biologically accurate temporal credit assignment
across the full behavioral timescale.
Beyond these corrections, v2 introduces an explicit thalamo-pallial loop
with a reticular nucleus (TRN) attention gate, D1/D2 striatal action selection
via disinhibition, theta-phase place cells with sequence learning, and
connectome-constrained retinal ganglion cell type targeting of tectal
layers~\cite{hildebrand2017,ahrens2013}.
The build plan proceeds in nine phases, each validated by a unit test before
the next phase begins.
Final evaluation compares v1 and v2 on the same five structured decision
scenarios (v1 score: 89/100) and a new set of biologically realistic behavioral
assays including prey capture, shoal cohesion, and dark-flash escape.
We predict that E/I balance alone will eliminate signal death and raise the
decision quality above 90/100; the full v2 system is expected to produce
emergent gamma oscillations, theta-phase coding, and behavioral repertoires
not achievable in the rate-coded v1 system.
\end{abstract}
...
This paper specifies the \textbf{v2 architecture}: the neuron model, layer
structure, connectivity, learning rules, and neuromodulation that address all
five limitations, with explicit biological justification at each step.
...
Section~\ref{sec:discussion} discusses the expected emergent properties.
...
\subsection{Phase 1 Pass Criterion}
...
\subsection{Phase 2 Pass Criterion}
...
ON-OFF looming   &  50 & SGC    & Size expansion      & Predator (Mauthner) \\
Direction selective & 50 & SO   & Optic flow          & Self-motion, orientation \\
\midrule
Total & 400 per eye & & & \\
\bottomrule
\end{tabular}
...
\paragraph{ON sustained (150/eye).}
Responds to luminance increases with sustained firing.
Implemented as RS neurons driven by the intensity channel:
$I_\text{ON} = G_\sigma * I_\text{raw}$ (Gaussian center-surround, $\sigma_c = 2°$, $\sigma_s = 6°$).

\paragraph{OFF transient (150/eye).}
Responds to luminance decreases with a transient burst.
Implemented as CH (chattering) neurons driven by $-\partial I/\partial t$,
detecting motion onset.

\paragraph{ON-OFF looming (50/eye).}
Responds to expanding dark discs (looming).
Implemented as IB neurons gated by the $\ell/v$ ratio
($\ell$ = half-size, $v$ = rate of change), with threshold $\ell/v < 10$\,ms.
Projects exclusively to SGC layer $\to$ Mauthner cell (C-start reflex).

\paragraph{Direction selective (50/eye).}
Responds to motion in a preferred direction via Reichardt detector:
$r_d = I(t-\delta) \cdot I(t)$ (delay $\delta = 5$\,ms).
Used for optic flow estimation (self-motion, orientation).
...
A drifting grating stimulus at 2\,Hz must produce:
(i) sustained SFGS-b response correlated with grating intensity;
(ii) transient SFGS-d bursts at onset/offset;
(iii) no SGC response (not looming);
(iv) DS cell tuning curve with $>2:1$ preferred:null ratio.
A looming stimulus (expanding disc, $\ell/v = 8$\,ms) must trigger SGC activity
within 20\,ms of threshold crossing.
...
``decision spike'' that commits the C-start: once fired, the 4-step motor
sequence (C-bend + power stroke) is executed without further neural input.
...
\begin{aligned}
  ...
  \text{Renshaw cells (RI)}: &\quad 4/\text{side} \quad\text{(new: recurrent inhibition)}
\end{aligned}
\end{equation}
Renshaw cells implement recurrent inhibition of motor neurons
($s_\text{post} \to \text{Renshaw} \to -s_\text{post}$),
producing the characteristic afterhyperpolarization and smooth burst termination
observed in zebrafish spinal cord~\cite{bhatt2009}.

\subsection{Phase 4 Pass Criterion}

(i) D1/D2 BG: high DA ($>0.7$) must produce $>80\%$ gate release;
low DA ($<0.3$) must produce $<20\%$ gate release.
(ii) Mauthner C-start: looming stimulus at threshold must trigger
M-cell spike within 5\,ms, completing motor sequence within 80\,ms.
(iii) CPG: 96-neuron circuit must produce stable L/R alternation at
0.5--5\,Hz with smooth speed modulation by descending drive.
...
\subsection{Phase 5 Pass Criterion}

(i) NA: amygdala activation ($\alpha > 0.6$) must raise all-layer gain $>30\%$.
(ii) 5-HT: a PATROL predator visible for $>30$ steps must reduce flee time
from $>60\%$ to $<30\%$ without manual EFE adjustment.
(iii) ACh: STDP weight change during high ACh must be $>2\times$ that during low ACh.
(iv) All four axes must be simultaneously active without numeric instability
across 1000 steps.
...
\subsection{Phase 7 Pass Criterion}

(i) With $\tau_e = 500$\,ms and DA pulse at $t + 400$\,ms, active synapses
at time $t$ must be strengthened by $>50\%$ of the instantaneous update value.
(ii) A cell pair with $t_\text{pre} - t_\text{post} = -10$\,ms (pre before post)
must show net LTP; $+10$\,ms must show net LTD.
(iii) After 1000 steps of random input, mean firing rates must stabilize
within $\pm 20\%$ of $\bar{r}^* = 0.1$ for all neurons (homeostatic scaling).
...
\subsection{Phase 8 Pass Criterion}

On the five v1 decision scenarios, v2 must score $\geq 85/100$
(v1 baseline: 89/100; some regression acceptable during architectural transition).
All five scenarios must pass (no regression below 70 on any single scenario).
...
zebrav2/
  ...
  tests/
    test_phase1_neuron.py
    test_phase2_ei.py
    test_phase3_sensory.py
    test_phase4_motor.py
    test_phase5_neuromod.py
    test_phase6_thalamus.py
    test_phase7_plasticity.py
    test_phase8_aif.py
    test_phase9_memory.py
    step29b_v1_vs_v2.py    # Final comparison
\end{verbatim}
...
%===================================================================
\section{Evaluation Plan: v1 vs.\ v2}\label{sec:evaluation}
...
\begin{table}[H]
\centering
\caption{Decision scenario evaluation: v1 vs.\ predicted v2 outcomes.}
\label{tab:scenario_comparison}
\begin{tabular}{llccp{4.5cm}}
\toprule
Scenario & Description & v1 Score & v2 Target & Key v2 mechanism \\
\midrule
A & Safe vs.\ risky food     & 100 & 100 & Preserved (GT danger zone) \\
B & Occluded food retrieval  &  85 &  90 & 5-HT patrol habituation (no manual EFE hack) \\
C & Predator charge          &  80 &  90 & Mauthner $<$20\,ms; sprint via NA \\
D & Starvation dilemma       & 100 &  95 & D1/D2 BG + ACh gated STALK avoidance \\
E & Obstacle detour          &  79 &  85 & Theta-phase sequence planning \\
\midrule
\textbf{Overall} & & \textbf{89} & \textbf{$\geq$92} & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Neural signatures: v1 (absent) vs.\ predicted v2 (emergent).}
\label{tab:neural_sigs}
\begin{tabular}{lll}
\toprule
Signature & v1 & v2 (predicted) \\
\midrule
...
We predict that v2 will achieve $\geq$92/100 decision rationality on the v1
benchmark while additionally exhibiting emergent gamma oscillations,
theta-phase sequence planning, 5-HT-mediated behavioral habituation,
and Mauthner-latency C-start escape---none of which are possible in
a rate-coded architecture.

%===================================================================
\section{Code Availability}

v1 source code: \texttt{zebrav1/} (brain modules in \texttt{zebrav1/brain/}).
v2 will be developed in \texttt{zebrav2/} alongside v1 for direct comparison.
