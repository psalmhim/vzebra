...
Run: python -m zebra_v60.tests.step26b_compare_wfb
"""
import os
import sys
import math
...
def main():
    ...
    cls_path = os.path.join(PROJECT_ROOT, "zebra_v60", "weights",
                            "classifier_v60.pt")
    wfb_path = os.path.join(PROJECT_ROOT, "zebra_v60", "weights",
                            "classifier_wfb_v60.pt")

    results = []

    # Run with original weights (classifier_v60.pt is default)
    print("\n--- Original (classifier_v60.pt) ---")
    r1 = run_eval("Original", weight_override=None)
    results.append(r1)

    # Run with W_FB-trained weights
    if os.path.exists(wfb_path):
        print(f"\n--- W_FB-trained (classifier_wfb_v60.pt) ---")
