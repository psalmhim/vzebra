...
66/100 to 75/100 (Table~\ref{tab:scenario_improvement}), with Scenario~C
(predator charge) rising from 30 to 98 and Scenario~A (safe food) from
49 to 90.  Scenario~E (obstacle detour) remains at 34, as the explore
goal requires separate investigation.
...
B & Occluded food retrieval        & 84  & 84  & A \\
C & Predator charge (survive)      & 30  & 98  & A \\
D & Starvation dilemma             & 70  & 70  & B \\
E & Obstacle detour                & 34  & 34  & D \\
\hline
\textbf{Overall} & & 66 & \textbf{75} & \textbf{B} \\
\hline
\end{tabular}
\end{table}
...
achieves 75/100 overall (Table~\ref{tab:scenario_improvement}), with
Scenario~C (predator charge) improving from 30 to 98 via the Mauthner
C-start reflex and GT flee direction override.
...
\caption{Decision rationality scores across 5 structured scenarios.
  A=90 (safe vs.\ risky food), B=84 (occluded food), C=98 (predator charge),
  D=70 (starvation dilemma), E=34 (detour).  Overall: 75/100 \textsc{rational}.}
\label{fig:scenarios}
\end{figure}
