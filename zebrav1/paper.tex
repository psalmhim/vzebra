...
\title{%
  ...
  \large Technical Report --- Zebrafish SNN v1 (Steps 1--29)
}
...
\end{equation}
When $n_{\text{stuck}} > 30$, the goal is overridden to EXPLORE for 20
steps.  During EXPLORE, the basal ganglia exploration multiplier
($\times 1.5$) drives wider random search arcs that naturally route
...
(Figure~\ref{fig:scenarios}).  After Steps~43--44 refinements the system
achieves 80/100 overall (Table~\ref{tab:scenario_improvement}), with
Scenario~C (predator charge) improving from 30 to 80 via the Mauthner
C-start reflex, and Scenario~E (obstacle detour) from 34 to 81 via
the forage-no-progress EXPLORE trigger.
...
\caption{Decision rationality scores across 5 structured scenarios.
  A=90 (safe vs.\ risky food), B=81 (occluded food), C=80 (predator charge),
  D=70 (starvation dilemma), E=81 (detour).  Overall: 80/100 \textsc{rational}.}
\label{fig:scenarios}
\end{figure}
...
The forty-two-step developmental curriculum with bidirectional homeostatic
...
Taken together, the forty-two steps demonstrate that the free energy
