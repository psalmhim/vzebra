# ============================================================
# FILE: pcsnn_v15_3_meta_gain.py
# AUTHOR: H.J. Park & GPT-5
# VERSION: v15.3 (2025-11-15)
#
# PURPOSE:
#     Demonstrates meta-precision (confidence) modulation of gain and memory.
# ============================================================
...
class PCSNNAI_Agent:
    ...
    def perceive_and_act(self, img):
        ...
        motive, gain_state, choice, gvec, td_error, conf = self.policy_field.step(
            dopa_out, rpe, F_total, z_mean.mean().item(), cms
        )
        ...
        return Fv, Fa, Fb, rpe, dopa_out, motive, gain_state, cms, eye, choice, mem_state.mean().item(), alpha_eff, conf
...
def main():
    ...
    Fv_hist, Dopa_hist, Mot_hist, Gain_hist, CMS_hist, Conf_hist, Mem_hist = [], [], [], [], [], [], []

    for t, (img, _) in enumerate(loader):
        Fv, Fa, Fb, rpe, dopa, motive, gain, cms, eye, choice, mem_mean, alpha_eff, conf = agent.perceive_and_act(img)

        if t % 50 == 0:
            print(f"Step {t:03d}: Fv={Fv:.3f} RPE={rpe:+.3f} Dopa={dopa:+.3f} Mot={motive:+.3f} "
                  f"Gain={gain:+.3f} Conf={conf:+.3f} CMS={cms:+.3f} Eye={eye:+.2f} Choice={choice} Mem={mem_mean:+.3f}")

        Fv_hist.append(Fv); Dopa_hist.append(dopa); Mot_hist.append(motive)
        Gain_hist.append(gain); CMS_hist.append(cms); Conf_hist.append(conf)
        Mem_hist.append(mem_mean)
        if t >= 300: break

    plt.figure(figsize=(10,8))
    plt.subplot(4,1,1); plt.plot(Fv_hist,label="Fv"); plt.legend()
    plt.subplot(4,1,2); plt.plot(Dopa_hist,label="Dopa"); plt.plot(Mot_hist,label="Mot"); plt.legend()
    plt.subplot(4,1,3); plt.plot(Gain_hist,label="Gain"); plt.plot(Conf_hist,label="Confidence"); plt.legend()
    plt.subplot(4,1,4); plt.plot(Mem_hist,label="Memory"); plt.legend()
    plt.tight_layout(); plt.savefig("plots/v15_3_meta_gain.png")
    print("[Saved] plots/v15_3_meta_gain.png")
