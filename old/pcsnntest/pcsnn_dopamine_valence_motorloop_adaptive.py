# ===============================================================
# pcsnn_dopamine_valence_motorloop_adaptive.py
# ===============================================================
...
# ===============================================================
# Retina: predictive coding + dopaminergic plasticity + exploration
# ===============================================================
class Retina(nn.Module):
    def __init__(self, n_in=784, n_lat=64, tau=0.9, eta=0.01,
                 pi_min=0.01, pi_max=2.0, beta_val=0.5,
                 lr_dopa=0.001, curiosity_decay=0.999, device="cpu"):
        super().__init__()
        self.device = device
...
# ===============================================================
# Recognizer with dopaminergic learning
# ===============================================================
class Recognizer(nn.Module):
    ...
    def dopamine_update(self, latent, pred, rpe, lr=0.002):
        if abs(rpe) < 1e-5: return
        with torch.no_grad():
            dW = rpe * torch.outer(latent.squeeze(),
                                   F.one_hot(torch.tensor(pred), 10).float().to(latent.device))
            self.fc.weight += lr * dW.T.clamp(-0.05, 0.05)

# ===============================================================
# Motor system + gaze control
# ===============================================================
class MotorSystem:
...
# ===============================================================
# Helper: shift input image by gaze angle
# ===============================================================
def shift_input(img, eye_angle):
    shift = int(eye_angle * 3)  # small horizontal shift
    return torch.roll(img, shifts=shift, dims=2)

# ===============================================================
# Agent combining retina, dopamine, recognizer, motor
# ===============================================================
class ZebrafishAgent:
    ...
    def perceive_and_act(self,img,label):
        # shift visual field by eye angle (gaze)
        img_shifted = shift_input(img, self.motor.eyeL)
        x = img_shifted.view(1, -1).to(self.device)
        ...
        self.recognizer.dopamine_update(retinal_act, pred, rpe)
...
def main():
    ...
    plt.tight_layout()
    plt.savefig("plots/dopamine_motorloop_adaptive.png")
    print("[Saved] plots/dopamine_motorloop_adaptive.png")

    # Plot category valences
    ...
    plt.savefig("plots/category_valence.png")
    print("[Saved] plots/category_valence.png")
