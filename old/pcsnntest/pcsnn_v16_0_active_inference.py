# ============================================================
# FILE: pcsnn_v16_0_active_inference.py
# AUTHOR: H.J. Park & GPT-5
# VERSION: v16.0 (2025-11-16)
#
# PURPOSE:
#     Demonstrates full Active Inference agent with EFE-based
#     policy inference and precision confidence.
# ============================================================
...
from modules.dopamine_system import DopamineEFESystem
from modules.basal_ganglia import BasalGanglia
from modules.policy_field import ActivePolicyFieldEFE
...
class ActiveInferenceAgent:
    def __init__(self, device="cpu"):
        ...
        self.dopa = DopamineEFESystem(device=device)
        self.policy = ActivePolicyFieldEFE(device=device)
    ...
    def perceive_and_act(self, img):
        ...
        # Expected free-energy estimation
        rpe, dopa, efe, prec = self.dopa.step(Fv, Fa, Fb, cms)
        efe_vec = [efe + 0.05*i for i in range(3)]  # placeholder per-goal EFE

        # Policy inference under Active Inference
        choice, gvec, post, conf = self.policy.step(efe_vec, cms)
        mem_state, _ = self.mem.step(z_mean, dopa, cms, gvec, gain_state=conf, rpe=rpe)
        eye = self.bg.step(efe_vec[0], efe_vec[1], dopa, rpe, cms)

        return Fv, Fa, Fb, efe, dopa, conf, cms, eye, choice
...
def main():
    ...
    EFE_hist, Dopa_hist, Conf_hist, CMS_hist, Eye_hist, Choice_hist = [], [], [], [], [], []

    for t, (img, _) in enumerate(loader):
        Fv, Fa, Fb, efe, dopa, conf, cms, eye, choice = agent.perceive_and_act(img)
        if t % 50 == 0:
            print(f"Step {t:03d}: EFE={efe:+.3f} Dopa={dopa:+.3f} Conf={conf:+.3f} CMS={cms:+.3f} Eye={eye:+.2f} Choice={choice}")
    ...
    plt.subplot(4,1,1); plt.plot(EFE_hist,label="Expected Free Energy"); plt.legend()
    plt.subplot(4,1,2); plt.plot(Dopa_hist,label="Dopamine"); plt.legend()
    plt.subplot(4,1,3); plt.plot(Conf_hist,label="Confidence"); plt.plot(CMS_hist,label="CMS"); plt.legend()
    plt.subplot(4,1,4); plt.plot(Choice_hist,label="Choice"); plt.yticks([0,1,2],["Left","Stay","Right"]); plt.legend()
    plt.tight_layout(); plt.savefig("plots/v16_0_active_inference.png")
    print("[Saved] plots/v16_0_active_inference.png")
