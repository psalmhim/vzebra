# pcsnn_dopamine_valence_pursuit_v11.py
...
# ============================================================
# RetinaPC — predictive coding retina with normalized learning
# ============================================================
class RetinaPC(nn.Module):
    ...
    def step(self, x, rpe=0.0, valence=0.0):
        pred = x @ self.W                           # latent prediction
        ...
        dW = 0.005 * (x.T @ err) * mod
        self.W.data += dW
        self.W.data /= (1.0 + self.W.data.abs().mean())  # normalize

        # bounded precision (0.01–2)
        self.Pi += 0.01 * (err.abs().mean() - self.Pi)
        self.Pi = torch.clamp(self.Pi, 0.01, 2.0)
        return pred.detach(), F_val.item(), self.Pi.mean().item()

# ============================================================
# Visual Cortex — cross-inhibitory hemispheric V1-like layer
# ============================================================
...
# ============================================================
# Working Memory — prefrontal integration buffer
# ============================================================
...
# ============================================================
# OpticTectum — midbrain saliency & gaze control (new)
# ============================================================
class OpticTectum:
    def __init__(self):
        self.eye_pos = 0.0
        ...
        self.center_pull = 0.02
    def step(self, valL, valR, Fmean):
        # saliency as valence difference weighted by precision signal
        drive = (valR - valL) - 0.3 * Fmean
        # velocity update with damping
        self.eye_vel = 0.9 * self.eye_vel + 0.1 * drive
        # eye position integration with central homeostasis
        self.eye_pos += self.eye_vel - self.center_pull * self.eye_pos
...
class PCSNNAI_Agent:
    ...
    def perceive_and_act(self, img, label):
        ...
        valL, valR, rpe = self.dopa.update(self.dopa.valL, self.dopa.valR,
                                           rewardL, rewardR)
        Fmean = (F_L + F_R) / 2
        eye = self.tectum.step(valL, valR, Fmean)
...
def main():
    ...
    plt.subplot(3,1,3)
    plt.plot(RPE, label="RPE"); plt.plot(Eye, label="Eye Position")
    plt.legend(); plt.xlabel("Step")
    plt.tight_layout(); plt.savefig("plots/v11_tectum_dynamics.png")
    print("[Saved] plots/v11_tectum_dynamics.png")
