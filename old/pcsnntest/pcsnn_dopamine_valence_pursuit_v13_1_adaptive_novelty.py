# ============================================================
# pcsnn_dopamine_valence_pursuit_v13_1_adaptive_novelty.py
# Zebrafish PC-SNN with thalamic relay, adaptive novelty mode
# Mode can be 'alternating' or 'exploratory'
# ============================================================

import torch, torch.nn as nn, torch.nn.functional as F
from torchvision import datasets, transforms
from torch.utils.data import DataLoader
import matplotlib.pyplot as plt, os, math, random
...
# ============================================================
# Thalamic relay (novelty detector)
# ============================================================
class ThalamusRelay:
    def __init__(self):
        self.state = 0.0
    ...
    def step(self, visual_err, audio_err):
        sal = abs(visual_err - audio_err)  # novelty as mismatch
...
class DopamineSystem:
    ...
    def update(self, pred_val_L, pred_val_R, reward_L, reward_R, crossmodal_salience=0.0):
        ...
        self.rpe = base_rpe + 1.0 * crossmodal_salience  # amplified novelty term
...
class BasalGanglia:
    ...
    def step(self, valL, valR, dopa, rpe):
        ...
        if self.mode == "alternating":
            self.alt_timer += 1
            if self.alt_timer % 70 == 0: drive *= -1.0
        else:  # exploratory mode
            switch_prob = torch.sigmoid(torch.tensor(abs(rpe) * 8.0))
...
class OpticTectum:
    ...
    def step(self, valL, valR, Fmean, bg_gate, dopa):
        ...
        # reflexive rebound from extreme gaze
        if abs(self.eye_pos) > 0.9:
            self.eye_vel *= -0.3
...
def main():
    ...
    agent = PCSNNAI_Agent(device=device, mode="alternating")  # or "exploratory"
    ...
    plt.tight_layout(); plt.savefig("plots/v13_1_adaptive_novelty.png")
    print("[Saved] plots/v13_1_adaptive_novelty.png")
