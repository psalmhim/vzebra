...
following (CPG persistence without social modulation), and heightened
habenula-driven strategy switching (frustration intolerance).
...
DA inflating the value of unrelated stimuli; (2)~\emph{working-memory
...
(3)~\emph{loose associations}---the classifier assigns high confidence to
...
\begin{table}[h]
\centering
\caption{Disorder phenotype profiles and biological analogs.}
\label{tab:disorders}
\begin{tabular}{lll}
\toprule
\textbf{Profile} & \textbf{Key parameters} & \textbf{Biological model} \\
\midrule
\texttt{wildtype}      & baseline               & control \\
\texttt{hypodopamine}  & DA=0.05, no phasic     & MPTP, reserpine \\
\texttt{asd}           & 5-HT=0.85, OXT$\downarrow$, social avoidance & shank3, SERT mutant \\
\texttt{schizophrenia} & NMDA$\times$0.3, DA=0.85 tonic & DISC1, ketamine \\
\texttt{anxiety}       & NA=0.75, flee thr=0.12 & CRF overexpression \\
\texttt{depression}    & DA=0.15, 5-HT=0.2, cortisol=0.6 & CMS model \\
\texttt{adhd}          & DA=0.35, ACh=0.25, high CPG noise & DA deficit model \\
\texttt{ptsd}          & amygdala gain$\times$2, cortisol=0.6 & trauma model \\
\bottomrule
\end{tabular}
\end{table}
